...
# ============================================================
    ...
    st.write("Visual outputs generated by the differential-expression and machine-learning pipelines.")

    visualization_files = {
        "Volcano Plot": "results/plots/volcano_plot.png",
        "MA Plot": "results/plots/ma_plot.png",
        "Top 10 Upregulated Genes": "results/plots/top_10_upregulated_genes.png",
        "Top 10 Downregulated Genes": "results/plots/top_10_downregulated_genes.png",
        "Top 20 DEG Heatmap": "results/plots/top_20_deg_heatmap.png",
        "PCA Plot": "results/plots/pca_plot.png",
        "Top DEG Heatmap": "results/visualization/top_DEG_heatmap.png",
        "ROC Curve": "results/roc_curve.png",
        "Confusion Matrix": "results/confusion_matrix.png",
        "Feature Importance": "results/top_20_feature_importance.png"
    }
    ...
    if available:
        ...
        st.warning("No visualization images were found.")
...
# ============================================================
    ...
    st.write("GO Biological Process and KEGG enrichment results for upregulated and downregulated genes.")

    enrichment_images = {
        "GO Biological Process — Upregulated": "results/enrichment/go_biological_process_upregulated_barplot.png",
        "GO Biological Process — Downregulated": "results/enrichment/go_biological_process_downregulated_barplot.png",
        "KEGG Pathway — Upregulated": "results/enrichment/kegg_pathway_upregulated_barplot.png",
        "KEGG Pathway — Downregulated": "results/enrichment/kegg_pathway_downregulated_barplot.png"
    }

    available = {
        name: path
        for name, path in enrichment_images.items()
        if os.path.exists(path)
    }

    if available:
        selected = st.selectbox(
            "Select enrichment visualization",
            list(available.keys())
        )
        st.image(
            available[selected],
        ...
        st.warning("Pathway enrichment visualizations were not found.")

    st.divider()
    st.subheader("Enrichment Result Tables")

    enrichment_tables = {
        "GO — Upregulated": "results/enrichment/GO_upregulated.csv",
        "GO — Downregulated": "results/enrichment/GO_downregulated.csv",
        "KEGG — Upregulated": "results/enrichment/KEGG_upregulated.csv",
        "KEGG — Downregulated": "results/enrichment/KEGG_downregulated.csv",
        "All — Upregulated": "results/enrichment/upregulated_enrichment.csv",
        "All — Downregulated": "results/enrichment/downregulated_enrichment.csv"
    }

    available_tables = {
        name: path
        for name, path in enrichment_tables.items()
        if os.path.exists(path)
    }

    if available_tables:
        selected_table = st.selectbox(
            "Select enrichment table",
            list(available_tables.keys())
        )
        table_df = load_csv(available_tables[selected_table])
        ...
        st.info("Enrichment CSV result files were not found.")
...
# ============================================================
    ...
    st.write("Protein-protein interaction networks and centrality-based hub-gene results from STRING.")
...
# ============================================================
    ...
    if available_ml:
        selected_ml = st.selectbox(
            "Select machine-learning visualization",
        ...
        st.warning("Machine-learning visualization files were not found.")

    st.divider()
    st.subheader("Selected Genes")

    ml_tables = {
        "Top 100 Selected Genes": "results/top_100_selected_genes.csv",
        "Top 100 Genes with Symbols": "results/top_100_genes_with_symbols.csv"
    }

    available_ml_tables = {
        name: path
        for name, path in ml_tables.items()
        if os.path.exists(path)
    }

    if available_ml_tables:
        selected_ml_table = st.selectbox(
            "Select machine-learning gene table",
            list(available_ml_tables.keys())
        )
        ml_df = load_csv(available_ml_tables[selected_ml_table])
        ...
        st.info("Machine-learning gene-selection tables were not found.")
